...
Milestone 2B is a `PROVISIONAL_REVIEW_CANDIDATE`. It demonstrates versioned clause expansion and an inspectable oracle-authoring workflow; it does not claim independently reviewed oracle truth, a frozen benchmark, or policy performance.
...
The explicit registry binds the six relation clauses to canonical contract and manifest fields. Its hash is recorded in every validation finding and the provisional manifest. An unavailable forbidden tool ID is intentionally schema-valid but semantically invalid, so the relation checker returns `CONTRACT_INVALID` rather than treating the fixture relationship as defective.
...
- `AGREE` with the independently reviewed state and set;
- `DISAGREE`, which requires adjudication before the unit is usable;
- `ADJUDICATED` with complete state and set values.

A completed review must also state whether it was performed without policy outputs. Pending review is honest intermediate state: even when the proposal matches computation, the unit remains `PENDING_REVIEW` and is not ready for scoring or freeze.
...
Before any v2 freeze decision, an independent reviewer must inspect each scenario without policy outputs, record agreement or disagreement, and complete adjudication where needed. Policy comparison, v2 counterfactual validation, broader scenario coverage, live-model evaluation, benchmark scaling, and Milestone 3 require separate authorization.
